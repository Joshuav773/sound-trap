...
export default function AdminUpload() {
  ...
  const [formData, setFormData] = useState({
    ...
    exclusivePrice: '',
    isFeatured: false,
    artistEmail: '', // For admin to select artist
    ...
    const checkAccess = async () => {
      ...
      if (userData) {
        ...
        if (parsedUser.role === 'admin' || parsedUser.role === 'artist') {
          ...
          // If admin, fetch artists for selection
          if (parsedUser.role === 'admin') {
            try {
              const res = await fetch('/api/users/artists')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (user?.role === 'admin') {
      data.append('isPendingStore', formData.isPendingStore.toString())
      if (formData.artistEmail) {
        data.append('artistEmail', formData.artistEmail)
      }
    }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          <Card className="bg-card border-border">
            ...
            <p className="text-muted-foreground">Upload your beats to SoundTrap</p>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="mt-2"
                      placeholder={user?.role === 'artist' ? user.name : "Producer name"}
                  ...
                  {user?.role === 'admin' && (
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setFormData({ ...formData, artistEmail: value })}
                      >
                ...
                {/* Pending Store Toggle - Admin Only */}
                {user?.role === 'admin' && (
                  <div className="flex items-center space-x-2">
                    <Switch
                      id="pendingStore"
                      checked={formData.isPendingStore}
                      onCheckedChange={(checked) => setFormData({ ...formData, isPendingStore: checked })}
                    />
                    <Label htmlFor="pendingStore">Add to Pending Store (prodbypending...)</Label>
                  </div>
                )}
